...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run apply-profile step.")
...
def main() -> None:
    ...
    profiled = apply_active_profile(
        base_result=base_result,
        profile_config_path=repo_path(args.profile_config),
    )
    counts = profiled.counts()
    print(
        "apply-profile complete: "
        f"profile={profiled.profile_name} ({profiled.profile_id}) "
        f"candidate_clusters={profiled.base_counts.get('candidate_clusters', 0)} "
    ...
    print(
        "metadata: "
        f"priority_sections={','.join(profiled.priority_sections)} "
        f"interests={','.join(profiled.interests)}"
    )

    limit = max(0, args.show_items)
    for item in profiled.items[:limit]:
        print(
